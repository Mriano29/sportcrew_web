...
export const FormButton: React.FC<CustomButtonProps> = ({
    ...
    const buttonStyle = thirdVariant ? variants.third : secondVariant ? variants.second : variants.default;

    return (
        <button type={type} onClick={onClick} className={`w-full py-2 px-4 rounded-md transition ${buttonStyle}`}>
            {title}
        </button>
    );
};